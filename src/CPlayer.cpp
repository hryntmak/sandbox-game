...
CPlayer::CPlayer(std::istream     &in,
    ...
    m_ActiveItem = CConfig::getInteger(in, "ActiveItem", 4);
    std::cout << "Player has been configured." << std::endl;
}

bool CPlayer::mine(std::unique_ptr<CBlock> & block) {
    int toolLevel = m_Inventory[m_ActiveItem] == nullptr ? 0 : m_Inventory[m_ActiveItem]->getToolLevel();
    // Check the possibility of mining
    if (block->getHardness() > toolLevel
     || block->getHardness() < 0)
        return false;
...
std::unique_ptr<CAction> CPlayer::action(const std::unique_ptr<CBlock> &block,
                                         bool                           steppedOn) {
    // If block ahead isn't solid and isn't stepped - use active item
    if(!block->isSolid() && !steppedOn) {
        std::unique_ptr<CAction> usageAction = std::make_unique<CNoAction>();
        if ( m_Inventory[m_ActiveItem] == nullptr
          || m_Inventory[m_ActiveItem]->use(usageAction) ) // If out of usage possibilities - delete item
...
std::ostream &operator<<(std::ostream  &out,
    ...
    for (int i = 0; i < 5; ++i) {
        if (player.m_Inventory[i] == nullptr) { // Isn't item on this slot
            out << i + 1 << ":  ";
            if (i!=4) // Isn't last block int the inventory
                out << ", ";
            continue;
        }
